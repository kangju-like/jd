...
const update = (col, query, query2) => {
    return new Promise(async(resolve, reject) => {
        ...
        collection.updateMany({...query }, { $set: {...query2 } }, function(err, result) { err ? reject(err) : resolve(result) })
    })
}
...
const remove = (col, query) => {
    return new Promise(async(resolve, reject) => {
        ...
        const db = client.db(dbName);
        const collection = db.collection(col);
        collection.deleteMany(query, function(err, result) { err ? reject(err) : resolve(console.log(result)) })
    })
}
...
const insert = (col, query) => {
    return new Promise(async(resolve, reject) => {
        ...
        const db = client.db(dbName);
        const collection = db.collection(col);
        collection.insertMany(query, function(err, result) { err ? reject(err) : resolve(result) })
    })
}
module.exports = {
    connect,
